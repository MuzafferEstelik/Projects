...
Write the query without using the OR operator. */
 SELECT * 
FROM  `Facilities` WHERE (`facid` =1) | (`facid` = 5) 
...
SELECT DISTINCT (firstname + ' ' + surname) AS member, Facilities.name AS facilityname
...
SELECT (`Members`.`firstname` + ' ' + `Members`.`surname`) AS member, 
	...
	END AS cost
		...
		`Bookings`.`starttime` < '2012-09-15 00:00:00' AND (
